...
default_num_groups <- c(3,4,5,6,7,8)

huc12_eflow_percents <- read.csv("data/HUC12s_eflow_percent.csv", stringsAsFactors = FALSE)
suffix<-"groups_num"

huc_data <- sf::st_as_sf(rgdal::readOGR(paste("data/phase2/cluster_species_level.gpkg", sep=""),"species_flowsens_3to8_notrans_nodelta", stringsAsFactors=FALSE))
...
species_maps <-function(groupname, title, region_data){
      ...
      huc_data_mapping <- huc_data %>% gather(key=model_run, value=grouping, paste(groupname,"_3",suffix, sep=""), paste(groupname,"_4",suffix, sep=""),paste(groupname,"_5",suffix, sep=""),paste(groupname,"_6",suffix, sep=""),paste(groupname,"_7",suffix, sep=""), paste(groupname,"_8",suffix, sep=""))
