...
species_maps <-function(groupname, title, region_data){
      ...
      huc_data_mapping <- huc_data %>% gather(key=model_run, value=grouping, paste(groupname,"_2",suffix, sep=""),paste(groupname,"_3",suffix, sep=""),paste(groupname,"_4",suffix, sep=""),paste(groupname,"_5",suffix, sep=""),paste(groupname,"_6",suffix, sep=""),paste(groupname,"_7",suffix, sep=""),paste(groupname,"_8",suffix, sep=""),paste(groupname,"_9",suffix, sep=""))
...
In this current version (June 2018) of the clustering, translocations were ignored, so clustering was done only on PISCES presence types 1 (observerd), 3 (expert opinion of extant range), and 9 (extirpated, but reintroduced). HUCs with taxa richness of less than 2 are also excluded as single species hucs may have their own specific rules. Clustering was done with Flow Sensitive Taxa.
...
huc_data <- sf::st_as_sf(rgdal::readOGR(paste("data/report_update/clusters.gpkg", sep=""),"june_2018_clusters_v2", stringsAsFactors=FALSE))
...
```{r map Desert, fig.height=12, fig.width=12, results='asis'}
